...
>
> 再修订(2026-09-22,AND-134):「不能创建独立条目」已被用户决定推翻。处理中发现与范围内条目
> 无关的问题,或用户在对话里直接要求时,AI 可以在指定产品里建**独立条目**;用户确认内容(含产品)
> 的要求不变。见 §3 D2 再修订。
...
| D2 | AI 能否创建条目 | ~~否~~ → 衍生条目可以(2026-09-15 修订)→ 独立条目也可以(2026-09-22 再修订) | 原理由:创建即 triage;注入内容可驱动 AI 批量造垃圾条目。修订见下 |
...
- ~~**只能衍生,不能凭空建。** `create_item` 必须带 `sourceItemKey`,且要通过与其他写工具相同的
  产品范围校验;新条目落在来源条目的产品里。~~ 已由 AND-134 推翻,见下方再修订。
...
### D2 再修订(2026-09-22,AND-134)

「只能衍生」在使用中挡住了两类正当需求:处理条目时发现的问题和范围内条目毫无关系,硬挂成衍生
会造出错误的「衍生自」关系;用户在对话里直接说「建一条」,却找不到可挂的来源条目。用户决定放开:

- **`create_item` 的 `sourceItemKey` 与 `productId` 必须且只能给一个。** 两个都给或都不给,服务端拒绝。
- **独立条目只在两种情况下建**:(a) 处理条目时发现与范围内条目都无关的问题;(b) 用户在对话里直接
  要求。和范围内条目有关的,仍按衍生条目建。这是 Skill 规则,服务端分辨不了。
- **产品范围照旧由服务端校验**:`productId` 必须在该账号 AI 可达的产品范围内。产品由 AI 经
  `list_products` 取得,不猜;展示给用户确认的内容里必须包含产品。
- **确认要求不变**:每一条都在会话里完整展示内容并拿到明确同意再调用,服务端仍然无法验证。
- **独立条目不关联任何条目**:不写 `derived_from_item_id`,来源侧没有 `derived_item_created` 事件;
  `item_created` 事件的 payload 带 `independent: true`,同样记为 `agent` 并带账号与客户端。
- **注入面的兜底**:同一产品每小时最多由 AI 建 10 条独立条目(衍生条目按来源条目另计),超出 429。
  幂等键的作用域为 `create_item:product:<productId>`。
