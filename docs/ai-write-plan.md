...
> 修订(2026-09-15,AND-50):第一条已部分放开,见 §3 D2。AI 可以在用户于会话里确认内容后,
> 从正在处理的条目上拆出**衍生条目**;不能创建与之无关的独立条目。
...
- ~~`create_item`。创建条目 = 决定什么值得做,属于人的判断。Web、Android 和浏览器内
  WebMCP(`apps/web/src/webmcp.ts:115`)已经覆盖录入场景。~~ 已由 AND-50 以「衍生条目」的
  形式放开,见 §3 D2。
- `update_item`。AI 不得覆盖人写的内容。分类建议通过评论表达,由人手动应用。
- 队列 / 连续处理模式(路线图 §6.5)。它天然是「AI 自己决定写哪些条目」,注入面最大。
- 字段建议的一键采纳按钮。先观察 AI 分类建议的实际质量,再决定值不值得做。
- 多用户与角色。（当时的判断。AND-33 已经落地多账号，但本文档描述的 AI 写入面不受影响：AI 令牌继承授权它的那个账号的产品权限，可写的动作没有变化。）
...
| D2 | AI 能否创建条目 | ~~否~~ → 衍生条目可以(2026-09-15 修订) | 原理由:创建即 triage;注入内容可驱动 AI 批量造垃圾条目。修订见下 |
...
### D2 修订(2026-09-15,AND-50)

实际使用中,处理一条条目时经常衍生出需要单独跟踪的事项;只写进评论,之后很难跟踪和回归。
用户决定放开,并提出硬性要求:每一条都要经用户确认内容之后才能创建。

- **确认放在 AI 会话里,不做服务端审批。** 这是用户在「服务端提案 + 人工批准」「AI 直接建进草稿」
  「只在会话里确认」三个选项中选定的。代价是**服务端无法验证用户是否真的确认过**:
  `create_item` 调用成功只说明 AI 调用了它。确认这一步是 Skill 承诺,不是服务端强制。
- **只能衍生,不能凭空建。** `create_item` 必须带 `sourceItemKey`,且要通过与其他写工具相同的
  产品范围校验;新条目落在来源条目的产品里。
- **编号照常顺序分配,另记「衍生自」关系**(`work_items.derived_from_item_id`)。不采用 `AND-50.1`
  这类子编号:编号的「前缀-整数」格式被排序、MCP 分页和 Mac 客户端的派单校验共同依赖。
- **注入面的兜底**:同一来源条目每小时最多由 AI 建 10 条;创建事件记为 `agent` 并带账号与客户端,
  界面上显示是哪个 AI 建的,不会冒充成人建的。
- 状态可以是草稿或待处理,与人在控制台建条目的规则一致(待处理需要平台)。
