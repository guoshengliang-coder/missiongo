...
MissionGo MCP 让经过鉴权的 AI 按用户给出的编号完整读取一个工作条目；开启写入后，还可以在条目上追加评论、领取待处理的条目、把 PR 已合并的条目推到待验证，以及在用户于会话里确认内容后创建衍生条目。
...
- `create_item`（同上）：从 `sourceItemKey` 拆出一条衍生条目，落在来源条目的产品里，照常顺序编号，
  两边都显示「衍生自」关系。`status` 必填（`inbox` / `ready`，后者需要 `platform`），同一来源条目每小时
  最多 10 条。**调用前必须在会话里拿到用户对完整内容的同意，这一点服务端无法验证，是 Skill 承诺。**
