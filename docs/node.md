...
「检测更新」也能立即检查并明确提示已经是最新版。有新版时会弹窗列出版本、发布时间，以及在
`release-notes/macos/` 里声明过的中文更新条目（与 macOS 无关的改动不出现）；只有点「同意更新」才安装，
点「稍后」会在菜单中保留提示。更新记录过长时在弹窗内滚动，按钮始终可见。
...
- Claude Code 控制台区分当前回合、等待回复和仅剩后台任务；显示回合与后台任务运行时间，以及宿主可取得的最近输出和思考数据。回合中发送的消息会排队，在下一回合交给 Claude Code；Codex 仍按原有方式即时引导。后台任务的「请求停止」会预填一条消息，发送后只表示请求已提交，任务是否停止由 Agent 后续回复确认。
- Agent 控制台聊天可单独发送附件，也可同时发送文字和最多 10 个文件。图片、视频、文档和日志沿用条目附件的格式及单文件大小限制；历史消息可预览图片、视频并下载原文件。Mac 节点须更新到支持聊天附件的版本，旧节点的会话只能继续发送纯文字。节点下载原文件后把本机文件路径交给 Claude Code、Codex 或 OpenCode；视频不会自动截帧或转写，能否直接查看取决于代理自身能力。
