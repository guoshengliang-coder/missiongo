...
- 多用户、角色与权限。（AND-33 之后已经实现：账号由管理员创建，按产品配置查看 / 操作 / AI 调用。此处保留为当时的判断记录。）
...
| 桌面首屏完整可见行数 | ~4 | — | **8** | ✅ |
| 移动首屏完整可见卡片（375×812） | 3.27 | ~~≥ 6~~ 已作废 | **2.3–4** | ⛔ |
...
**移动端密度指标已作废（AND-6）。** 上线后的实际反馈是：手机上摘要被隐藏、缩略图小到认不出，
一屏 6 条换来的是每条都看不懂。指标背后的假设——「手机也是用来扫一遍待办的」——是错的；
手机上的动作是「看清楚某一条」，扫读发生在 PC。移动端因此恢复两行摘要、缩略图放大到 84×96、
`+N` 改为角标，卡片回到 112–236px。PC 端不动，一屏仍是 11.5 条。
