...
| 批次 | 主题 | 条目 | 服务端改动 | 说明 |
|---|---|---|---|---|
| P0 | 接出已有能力，把话说对 | UI-01 … UI-06 | 无 | 全部几十行以内，覆盖全部阻断与高优先级问题 |
| P1 | 列表与表单重构 | UI-07 … UI-12 | 仅 UI-08 | 界面结构调整，收益最大的一批 |
| P2 | 系统化，消除第二套实现 | UI-13 … UI-17 | UI-17 | 有重构成本，可跨版本 |
...
- **B（单独立项）** 开能力：`app.ts:287` 接一个环境变量传入 `enableWriteTools`，同步 README、`docs/mcp-contract.md`、`docs/ai-client-setup.md` 与 `skills/missiongo/SKILL.md`。写工具一旦开出去就是接口承诺，不适合夹在界面迭代里。
...
| UI-14 | 桌面端 ≥1280px 改「列表 + 详情」双栏；顺带删掉 `listScrollTopRef` 那套手工滚动补偿 | `App.tsx`、`styles.css` |
| UI-15 | `SdkFeedback` 复用 `WorkItemFields` 并接入 i18n。当前是第二套表单实现，且文案全部硬编码中文 | `apps/web/src/SdkFeedback.tsx` |
| UI-16 | 三处灯箱收敛为一个基于 `<dialog>` 的组件，一次性拿到焦点陷阱、Esc 与焦点归还 | `App.tsx` |
| UI-17 | 补齐删除 / 归档链路（产品、模块、条目的软删除与恢复），与 `cancelled` 形成完整的「收回」语义 | `store.ts`、`app.ts`、`App.tsx` |
...
| 指标 | 现状（实测） | 目标 |
|---|---|---|
| 桌面一屏可见条数（1440×900） | 5.3 | ≥ 9 |
| 移动一屏可见条数（375×812） | 3.27 | ≥ 6 |
| 最小字号 | 7px | 11px |
| 文字最低对比度 | 2.88 : 1 | ≥ 4.5 : 1 |
| 灵感表单高度 | 1002px | ≤ 560px |
| 录入表单徽章数 | 11 | ≤ 3 |
| 打开「记录」到可打字 | 8 次 Tab | 0 |
| REST 已实现但界面无入口的能力 | 3 | 0 |
| 界面文案与 README 的能力口径 | 不一致 | 一致 |

对比度按 WCAG 相对亮度公式，用 `getComputedStyle` 的计算值算，不目测。
