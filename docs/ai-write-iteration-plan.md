...
> 文档版本：v1.0
> 状态：第一刀已实现,待部署。第 0–5 段全部完成;开启写入档位是部署动作,尚未执行
> 日期：2026-09-06
> 依据：`feat/ui-iteration` @ `f216c94` 的代码通读

本文件只排**第一刀(评论闭环)**的实施顺序。字段级的写入约束见
[AI 写入规则](ai-write-rules.md)。能力边界、原则和 15 条决策见
...
|---|---|---|
| S0-1 | 提交本文件 | — |
| S0-2 | AW-05 写工具档位化 + 守卫加固 | 小 |

S0-2 **对外行为零变化**——今天没有任何调用方传过 `enableWriteTools`,默认档 `none` 与现状
完全一致。把它单独落,是为了让第一刀里唯一碰安全守卫的部分被单独审,而不是混在评论功能里。

### 第 1 段：合并发布

`feat/ui-iteration` 合并进 `main` 并发布。这是 [设计 §11](ai-write-plan.md) 定的启动条件。

### 第 2 段：A 批,服务端地基

| 提交 | 内容 | 体量 | 备注 |
|---|---|---|---|
| C1 | AW-01 事件归属字段(迁移 13) | 中 | 改 `insertEvent` 全部调用点 |
| C2 | AW-02 评论模型 + `appendAnalysis` 归并(迁移 14) | 大 | **动前先跑 `scripts/backup.mjs`** |
| C3 | AW-03 评论 REST 面 + `openapi.yaml` | 中 | |
| C4 | AW-04 `missiongo:write` scope | 中 | 动 OAuth 授权页与令牌签发 |

**验证点**:C3 之后可用 `curl` 打通「发评论 → 读回 → 撤回」;C2 之后必须确认历史
`analysis_appended` 的内容一条不少。

### 第 3 段：B 批,MCP 与 Skill

| 提交 | 内容 | 体量 |
|---|---|---|
| C5 | AW-06 `append_comment` 工具 | 中 |
| C6 | AW-07 `get_current_account` 返回能力发现 | 小 |
| C7 | AW-08 Skill 2.0.0(维持单文件) | 中 |

**验证点**:用一个真实 MCP 客户端连一次,确认只读 token 看不到评论工具、授权后能写能读回。

### 第 4 段：C 批,Web

| 提交 | 内容 | 体量 |
|---|---|---|
| C8 | AW-09 时间线接入评论 + 人机来源区分 | 中 |
| C9 | AW-10 评论输入 + AW-11 软撤回界面 | 中 |

**验证点**:人机闭环成立——AI 写一条,你在 Web 上回一条,再把 AI 那条撤回。
...
放最后,因为它把仓库里的「当前只读」改成「可写」。做早了,文档会先于能力撒谎。
...
第一刀的任何一段都**可以随时部署**:写能力默认关闭,`none` 档下 MCP 的工具清单和今天一模一样。
真正的发布动作是**翻那个环境变量**,而它应该等到第 4 段做完——在那之前打开,等于让 AI 往一个
人看不见的地方写东西。

### 如果时间不够

天然的收缩线是 AW-11(软撤回界面)。评论功能没有它也成立,可以作为紧随其后的一刀。
但不要拖太久:撤回存在的理由是 AI 写的内容会喂回给下一次读取,一条错的分析删不掉就会被反复引用。
