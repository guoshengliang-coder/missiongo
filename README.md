...
项目目前适合自行部署和小范围协作。它不是面向公众开放注册的 SaaS：账号由管理员创建，没有注册入口。默认不向第三方发送工作内容；管理员配置 DeepSeek API Key 后，用户主动点击「AI 提炼」会将当前需求内容文字发送给 DeepSeek 生成标题，附件不会发送。
...
- 已连接 AI 客户端的列表与逐个撤销；
- 可选的 DeepSeek 标题提炼：管理员在「账户」中配置全站 API Key；点击按钮时仅发送需求内容文字，生成的标题仍可编辑；
