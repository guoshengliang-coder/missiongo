...
项目目前适合自行部署和小范围协作。它不是面向公众开放注册的 SaaS：账号由管理员创建，没有注册入口。默认不向第三方发送工作内容；管理员配置 DeepSeek API Key 后，用户主动点击「AI 提炼」会将当前需求内容文字发送给 DeepSeek 生成标题。管理员还可另行启用 Agent 会话待办分类，此时只发送最新 AI 消息和必要会话状态。两项功能都不发送附件或日志，自动分类默认关闭。
...
- 已连接 AI 客户端的列表与逐个撤销；
- 可选的 DeepSeek 标题提炼与 Agent 会话待办分类：标题提炼只在用户点击时发送需求文字；待办分类使用独立开关且默认关闭，启用后仅发送最新 AI 消息和必要会话状态；
