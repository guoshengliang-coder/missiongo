...
- 已连接 AI 客户端的列表与逐个撤销；
- Agent 控制台会话可向 Claude Code、Codex 和 OpenCode 发送文字、图片、文件和视频，并在历史消息中预览或下载附件；
...
生产构建面向 Chrome/Edge 90+、Firefox 90+ 和 Safari/iOS 15.4+。低于 1024px 时切换为单栏布局，并适配横屏、安全区和底部手势区域；触屏设备不论宽度都使用 44px 的点击区域。离线能力仅覆盖应用壳层、已缓存页面和本地草稿，不缓存 API、管理员凭据或受保护附件，也不会在离线时排队提交。
