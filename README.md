...
项目目前适合自行部署和小范围协作。它不是面向公众开放注册的 SaaS：账号由管理员创建，没有注册入口。未配置 DeepSeek 时不向第三方发送工作内容；配置 DeepSeek API Key 后，用户主动点击「AI 提炼」会将当前需求内容文字发送给 DeepSeek 生成标题，Agent 会话待办分类也会默认开启并只发送最新 AI 消息和必要会话状态，管理员可单独关闭自动分类。两项功能都不发送附件或日志。
...
- 搜索、筛选、分页、编辑、状态流转和人工验收，把条目退回待处理时必须填写原因；
- 简体中文与英文界面，以及基础 PWA 离线壳层和本地草稿恢复；
- 邮箱账号登录，账号由管理员创建，无公开注册入口；
- 账号昵称：用于评论和时间线署名，留空则显示邮箱 `@` 前面的部分；
- 管理员与普通用户两种角色，账号 × 产品的查看 / 操作 / AI 调用权限可从账号和产品两侧配置；
- 产品创建者自行授予和移除该产品的权限：按邮箱添加成员，直接生效，不需要对方同意；
- 已连接 AI 客户端的列表与逐个撤销；
- 可选的 DeepSeek 标题提炼与 Agent 会话待办分类：标题提炼只在用户点击时发送需求文字；配置密钥后待办分类默认开启，管理员可用独立开关关闭，分类时仅发送最新 AI 消息和必要会话状态；
...
- 公开注册、团队与组织；
- 授予或移除权限时给对方的通知（目前静默生效），以及产品授权变更的审计记录；
- 忘记密码找回；
- macOS/iOS 反馈 SDK；
...
生产构建面向 Chrome/Edge 111+、Firefox 113+ 和 Safari/iOS 16.2+（Android 需要对应版本的 System WebView），低于此版本的浏览器会看到「浏览器版本过低」的提示而不是残缺的页面。低于 1024px 时切换为单栏布局，并适配安全区和底部手势区域（横屏适配尚不完整，见 [UI/UE 复盘](docs/ui-ue-review-2026-09.md) C1、C2）。离线能力仅覆盖应用壳层、已缓存页面和本地草稿，不缓存 API、管理员凭据或受保护附件，也不会在离线时排队提交。
