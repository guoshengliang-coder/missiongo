...
H5 编辑器的标题旁有「AI 提炼」按钮：填写现象描述后可让 DeepSeek 生成可编辑标题。
只有描述文字会发送给 DeepSeek，附件、日志与设备上下文不会发送。此功能由 MissionGo
Web 与服务端提供，已接入 `openFeedback` 的宿主无需更新 SDK 或重新打包 App；管理员需先
在 MissionGo 账号设置中配置 DeepSeek API Key。程序化提交不会自动生成标题。
