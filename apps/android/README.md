...
2×2 小组件（可拉伸到 4×2）显示全部产品中「待我处理」的 Agent 会话数、运行中与异常会话数，以及「待处理」条目数（AND-149）。

- 数据来自 `GET /api/v1/widget/summary`，沿用 WebView 已保存的登录 Cookie，不另设凭据；服务端返回 401 时显示「登录已过期」。
- 刷新入口有三个，共用同一段逻辑：约 15 分钟一次的 WorkManager 周期任务、离开 App 时、点小组件右上角的刷新胶囊。刷新失败保留上次的数字并注明数据时间。
