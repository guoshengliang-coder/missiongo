...
export class MissionGoStore {
  ...
  /**
   * Take a ready item into progress. This is the only transition an agent may
   * make, and it needs no lease: the status is the lock, because only a ready
   * item can be claimed. A session that dies mid-work leaves the item in
  ...
  claimWorkItem(input: ClaimWorkItemInput): WorkItemSnapshot {
    ...
    const operation = `claim_item:${input.itemKey.toUpperCase()}`;
    ...
    return this.database.transaction(() => {
      ...
      if (item.status !== "ready") {
        throw conflict("item_not_claimable", "Only a ready work item can be claimed.");
      }
      const now = new Date().toISOString();
      this.applyTransition(item, "in_progress", "agent", "claim", agentId, now, input.attribution ?? {});
